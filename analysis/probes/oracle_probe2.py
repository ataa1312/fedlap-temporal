...
print(f"=== probe2 uci ({len(res['FUT_cos'])} snaps) — CUR = reconstruct current graph (sanity), FUT = predict t+1 links ===")
for k in ["CUR_cos","CUR_heat1","CUR_heat10","CUR_dot","FUT_cos","FUT_heat1","FUT_heat10","FUT_dot"]:
    v=[x for x in res[k] if x is not None]
    print(f"  {k:10s} AUC={st2.mean(v):.3f}")
