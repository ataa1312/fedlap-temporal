"""usage: python analysis/probes/cond_probe5.py [dataset] [config] [Cs] [seeds]"""
...
PERM = np.random.default_rng(42).permutation(N) if False else None
...
for t in range(T - 1):
    ...
    def spec(pairs, Q):
        ua = Q[[a for a, _ in pairs]]; ub = Q[[b for _, b in pairs]]
        return (ua * ub).sum(1)
    cand_t[t] = (pos, neg, spec(pos, Qnorm), spec(neg, Qnorm),
                 spec(pos, Qplac), spec(neg, Qplac))
    ...
    for (u, v) in pos:
        ...
        while picked < NEG_RANK:  # pad in the unlikely exhaustion case
            ...
            picked += 1
    rank_t[t] = (len(pos), rank_pairs, spec(rank_pairs, Qnorm), spec(rank_pairs, Qplac))
...
for C in CS:
    ...
    for seed in SEEDS:
        ...
        per_t = {k: [] for k in ["mrr_model", "mrr_fused", "mrr_fused_plac",
                                 "auc_model", "auc_fused", "auc_fused_plac", "mrr_spec"]}
        ts = sorted(set(capt) & set(cand_t))
        for i, t in enumerate(ts):
            past = ts[:i]
            if len(past) < WARM:
                continue
            pos, neg, sp_p, sp_n, pl_p, pl_n = cand_t[t]
            ms = capt[t][0]
            ...
            def fit(idx):
                ...
                for tp in past:
                    row = cand_t[tp]
                    po, ne = row[0], row[1]
                    spp, spn = row[idx], row[idx + 1]
                    m = capt[tp][0]
                    X += [np.column_stack([m[:len(po)], spp]),
                          np.column_stack([m[len(po):], spn])]
                    y += [np.ones(len(po)), np.zeros(len(ne))]
                return make_pipeline(StandardScaler(), LogisticRegression(max_iter=500)).fit(
                    np.vstack(X), np.concatenate(y))
            clf_r, clf_p = fit(2), fit(4)
            fu = lambda c, m_, s_: c.predict_proba(np.column_stack([m_, s_]))[:, 1]
            per_t["auc_fused"].append(auc(fu(clf_r, ms_p, sp_p), fu(clf_r, ms_n, sp_n)))
            per_t["auc_fused_plac"].append(auc(fu(clf_p, ms_p, pl_p), fu(clf_p, ms_n, pl_n)))
            npos, rpairs, rspec, rplac = rank_t[t]
            rms = capt[t][1]
            per_t["mrr_model"].append(mrr_from_scores(npos, rms))
            per_t["mrr_fused"].append(mrr_from_scores(npos, fu(clf_r, rms, rspec)))
            per_t["mrr_fused_plac"].append(mrr_from_scores(npos, fu(clf_p, rms, rplac)))
            per_t["mrr_spec"].append(mrr_from_scores(npos, rspec))
        for k in per_t:
            if per_t[k]:
...
for C in CS:
    ...
    print(f"{C:>2d} {ms_('mrr_model'):>15s} {dd('mrr_fused','mrr_model'):>17s} "
          f"{dd('mrr_fused_plac','mrr_model'):>17s} {ms_('mrr_spec'):>15s} "
          f"{dd('auc_fused','auc_model'):>15s} {dd('auc_fused_plac','auc_model'):>15s}")
